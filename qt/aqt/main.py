...
class MainWebView(AnkiWebView):
    def __init__(self, mw: AnkiQt) -> None:
        # Manifold shell: the main web view hosts the Manifold SPA, so it is
        # registered as the MANIFOLD kind to receive backend API access (see
        # AnkiWebPage._profileForPage), mirroring how the stats dialog's
        # DECK_STATS view gets it.
        AnkiWebView.__init__(self, kind=AnkiWebViewKind.MANIFOLD)
        self.mw = mw
...
class AnkiQt(QMainWindow):
    ...
    def loadCollection(self) -> bool:
        ...
        try:
            ...
            # Manifold shell: boot into the Manifold home instead of Anki's
            # deck browser. The collection, sync and close machinery above and
            # below is untouched; only the initial view is redirected.
    ...
    def _manifoldState(self, oldState: MainWindowState) -> None:
        # Manifold shell: hide Anki's native chrome and load the Manifold home
        # into the main web view. In-app navigation (home, dashboard, session)
        # is handled by the Manifold web app's own links.
        self.hide_native_chrome()
        self.web.load_sveltekit_page("manifold")
        self.web.setFocus()
...
title="{}" {}>{}</button>""".format(
    ...
    def setupKeys(self) -> None:
        # Manifold shell: Anki's single-letter global shortcuts are dropped.
        # "d"/"s" would swap the main view back to the deck browser/reviewer,
        # and "a"/"b"/"t"/"y" collide with the Manifold session's own A-E key
        # bindings. Only the debug console (Ctrl+:) is kept. Sync still runs via
        # the auto-sync-on-open/close path, not a shortcut.
        globalShortcuts = [
    ...
    def hide_native_chrome(self) -> None:
        """Manifold shell: suppress Anki's native chrome.

        Hides the top toolbar, the menu bar and the bottom toolbar so the deck
        list, reviewer and menus are never shown. These widgets keep their
        underlying machinery (sync, profile, collection) intact; only their
        visibility is suppressed. Reversible: call show()/show_menubar() to
        restore. On macOS the system menu bar is native and stays as the OS
        manages it, matching Anki's own behaviour in fullscreen mode.
        """
        self.form.menubar.setVisible(False)
        self.toolbarWeb.setVisible(False)
        self.bottomWeb.setVisible(False)
